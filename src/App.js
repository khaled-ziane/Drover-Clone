...
function App() {
  ...
  const sortHandler = (e) => {
    if (e.target.value == "Price - Low to High") {
      ...
      setload(-1);
      const newasc = sort.sort(function (a, b) {
      ...
      setload(1);
      const newdsc = sort.sort(function (a, b) {
    ...
    if (e.target.value == "1 to 6 months") {
      ...
      const newTableFilter = data.filter(
        (car) => 1 <= car.duration && car.duration <= 6
      );
      ...
      setclose(true);
      const newTableFilter = data.filter(
        (car) => 7 <= car.duration && car.duration <= 12
      );
      setsort(newTableFilter);
      ...
      setclose(true);
      const newTableFilter = data.filter(
        (car) => 13 <= car.duration && car.duration <= 18
      );
      setsort(newTableFilter);
    ...
    if (e.target.value >= 3 && e.target.value <= 30) {
      ...
      const newTableFilter = sort.filter(
        (car) => car.price <= changeValue * 50
      );
      setsort(newTableFilter);
    }
  };
  ...
  const searchInputHandler = (e) => {
    ...
    console.log(val);
    val = val.toLowerCase();
    const newTableFilter = data.filter(function (car) {
